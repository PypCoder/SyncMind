...
if __name__ == "__main__":
    app = SyncMindApp()
    app.mainloop()
c. Add logic to open the room:

def open_room_action(self):
    selected = self.room_list.get(tk.ACTIVE)
    if selected == "Subhan’s Room":
        self.show_subhan_room()

def show_subhan_room(self):
    room_window = tk.Toplevel(self)
    room_window.title("🧠 Subhan’s Room")
    SubhanRoom(room_window).pack()
